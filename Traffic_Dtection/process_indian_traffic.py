...
# Path to INDRA dataset folder (update this path)
dataset_folder = r"Videos"

# Get all .MOV video files
video_files = [f for f in os.listdir(dataset_folder) if f.lower().endswith(".mov")]

# Traffic signal threshold
THRESHOLD_HIGH = 5  # Vehicles needed to turn green
THRESHOLD_LOW = 2   # Vehicles must reduce to this to turn red
...
for video in video_files:
    ...
    while cap.isOpened():
        ...
        for box in results[0].boxes:
            ...
            if cls_id in [2, 3, 5, 7]:  # Count only cars, buses, trucks
                ...
                for i, lane in enumerate(filtered_lanes):
                    x1_lane, _, x2_lane, _ = lane
                    if x1_lane <= x_center <= x2_lane:
                        lane_counts[i] += 1
        ...
        cv2.imshow("INDRA Traffic Signal Control", frame)
